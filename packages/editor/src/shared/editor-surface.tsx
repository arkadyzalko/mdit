...
export function EditorSurface({
	...
	return (
		<Plate editor={editor} onValueChange={onValueChange}>
			<PlateContainer
				className={cn(
					"ignore-click-outside/toolbar relative w-full h-full overflow-y-auto caret-primary select-text selection:bg-brand/14 focus-visible:outline-none",
			...
			>
				<PlateContent
					className={cn(
						"group/editor relative size-full min-h-[calc(100vh-3rem)] overflow-x-hidden wrap-break-word whitespace-pre-wrap rounded-md px-8 pt-4 pb-72 text-base select-text text-foreground ring-offset-background font-scale-scope focus-visible:outline-none md:pt-16 sm:[padding-left:max(64px,calc(50%-350px))] sm:[padding-right:max(64px,calc(50%-350px))]",
